...
result = reverse_asin_research(
    ...
    asins=["B08N5WRWNW"],            # 1–10 ASINs (plain ASIN, without us/ prefix)
    output_dir="/round-{N}/data",
    page_size=100,                   # Max rows collected per ASIN (default 100)
...
- Call `client.keywords_by_asin(asin=asin, page_size=100)` for each ASIN
