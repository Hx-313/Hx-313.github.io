...
import sys
sys.path.insert(0, '/icbu-amazon-market-intel/scripts/product_database')
from market_opportunity_discovery import discover_opportunities
result = discover_opportunities(mcp_data=<mcp_response>, output_dir="/round-{N}/data")
```
...
- Call `client.product_database(categories=[cat], include_keywords=[kw], page_size=50)` for each category
