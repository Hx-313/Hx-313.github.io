...
import sys
sys.path.insert(0, '/icbu-amazon-market-intel/scripts/sales_estimates')
from promotion_impact import analyze_promotion_impact

result = analyze_promotion_impact(
    mcp_data=<mcp_response>,
    asins=["B0D1XD1ZV3", "B0BSHF7WHW"],
    promo_start="2025-11-24",              # Promotion start date
    promo_end="2025-12-02",                # Promotion end date
    baseline_days=14,                       # Pre-promotion baseline days (default 14)
    recovery_days=7,                        # Post-promotion recovery days (default 7)
