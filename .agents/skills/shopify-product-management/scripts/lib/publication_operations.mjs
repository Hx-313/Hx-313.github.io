...
export const PRODUCT_PUBLICATION_STATE_QUERY = `
query GetProductPublicationState($id: ID!, $publicationId: ID!) {
  product(id: $id) {
    id
    title
    status
...
export function normalizeProductPublicationInput(raw) {
  if (!isObject(raw)) throw fail("set_product_publication input must be an object")
...
function readProductPublicationState({ store, id, publicationId, executor }) {
  const result = execute(executor, {
    store,
    query: PRODUCT_PUBLICATION_STATE_QUERY,
  ...
  return {
    ...
    published: result.product.publishedOnPublication === true,
  }
}

export function setProductPublication({ store, input, apply, executor = runShopifyOperation }) {
  const normalized = normalizeProductPublicationInput(input)
  const publication = resolvePublication({ store, target: normalized.publication, executor })
  const before = readProductPublicationState({
    store,
    id: normalized.id,
  ...
  const query = desiredPublished ? PUBLISH_PRODUCT_MUTATION : UNPUBLISH_PRODUCT_MUTATION
  ...
  const after = readProductPublicationState({
    store,
    id: normalized.id,
    publicationId: publication.id,
    executor,
  ...
  if (after.published !== desiredPublished) {
    throw fail("Product publication verification did not match the requested outcome", "verification_failed", {
