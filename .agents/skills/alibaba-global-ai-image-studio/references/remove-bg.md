---
name: remove-bg
description: 消除商品图片背景，生成白底图，结果路径保存在json中。通过 mcp_call 工具调用背景消除服务处理图片，输入一张图片，输出背景已移除的处理后图片路径。当用户需要去除图片背景、生成白底商品图、处理产品照片时使用。
---
...
mcp_call {
  ...
  "arguments": {
    "abilityCode": "ggsImageExtraction"
    "imageUrl": "<图片URL>",
    "prompt": ""
  }
}
...
**参数说明：**
- `abilityCode`：固定值 `ggsImageExtraction`（白底图能力代码）
- `imageUrl`：输入图片的URL地址
- `prompt`：固定为空字符串
