---
name: remove-bg
description: 消除商品图片背景，生成白底图，结果路径保存在json中。调用背景消除服务处理图片，输入一张图片，输出背景已移除的处理后图片路径。当用户需要去除图片背景、生成白底商品图、处理产品照片时使用。
---
...
本 Skill 用于消除商品图片的背景，生成干净的白底图，适用于电商产品展示。
...
| 必传参数 | 校验规则 | 缺失时处理 |
|---------|---------|-----------|
| `abilityCode` | 固定值 `imageExtraction`，不可为空 | 终止流程，提示"系统参数异常，请重试" |
| `imageUrl` | 用户提供的图片 URL，不可为空 | 终止流程，提示"请提供需要去除背景的图片" |

### 3. 调用 workctl CLI（两步调用）

背景消除需要调用两个 workctl 命令，按顺序执行：
...
参数：
  abilityCode: "imageExtraction"
  imageUrl: "<图片URL>"
  prompt: ""
```

**参数说明：**
- `abilityCode`：固定值 `imageExtraction`（白底图能力代码）
- `imageUrl`：输入图片的URL地址
- `prompt`：固定为空字符串
