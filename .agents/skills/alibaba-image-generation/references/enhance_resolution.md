...
description: 提升原图的清晰度，生成对应的高清图，结果路径保存在json中。调用清晰度增强服务处理图片，输入一张图片，输出清晰度提升后的处理后图片路径。当用户需要提升图片分辨率、增强图片清晰度、生成高清图时使用。
---
...
本 Skill 用于提升原图的清晰度，生成对应的高清图，适用于需要高分辨率图片的场景。

## 使用流程

### 1. 准备输入

- 输入：一张图片（支持常见格式：JPG、PNG、WEBP 等）
- 图片路径：用户提供的图片文件路径
...
| 必传参数 | 校验规则 | 缺失时处理 |
|---------|---------|-----------|
| `abilityCode` | 固定值 `imageHighDefinition`，不可为空 | 终止流程，提示"系统参数异常，请重试" |
| `imageUrl` | 用户提供的图片 URL，不可为空 | 终止流程，提示"请提供需要增强清晰度的图片" |

### 3. 调用 workctl CLI（两步调用）

清晰度增强需要调用两个 workctl 命令，按顺序执行：
...
参数：
  abilityCode: "imageHighDefinition"
  imageUrl: "<图片URL>"
  prompt: ""
```

**参数说明：**
- `abilityCode`：固定值 `imageHighDefinition`（高清增强能力代码）
- `imageUrl`：输入图片的URL地址
- `prompt`：固定为空字符串
