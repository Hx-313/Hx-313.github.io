...
def _normalize_product_row(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    # GGS FIX: Only auto-generate Amazon prodUrl if the product is confirmed to be from Amazon
    # (i.e., has a valid ASIN starting with 'B0' or 10-char alphanumeric).
    # Do NOT blindly generate Amazon URLs for Alibaba product IDs.
    if not row['prodUrl'] and row['asin']:
        import re as _re
        asin_val = str(row['asin']).strip()
        if _re.match(r'^B0[A-Z0-9]{8}$', asin_val):
            # Valid Amazon ASIN format — safe to generate Amazon URL
            row['prodUrl'] = f"https://www.amazon.com/dp/{asin_val}"
        # Otherwise leave prodUrl empty to avoid cross-platform URL mismatch
    return row
