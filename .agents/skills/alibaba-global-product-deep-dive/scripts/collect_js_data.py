...
Step 2: Convert raw JSON data from Jungle Scout MCP tools into CSV files.
The Agent calls MCP tools (js_*) via mcp_call, saves the raw JSON response
to files using write_file, then this script handles JSON → CSV transformation.

Supports both raw MCP responses ({"data": [...], "links": ..., "meta": ...})
and pre-unwrapped data arrays. Each save_* function auto-unwraps the envelope.
...
def _unwrap_response(data):
    """Unwrap MCP/API response envelope: {"data": [...]} → [...]"""
...
def save_product_database_csv(data, output_dir):
    ...
    for item in (data or []):
        ...
        rows.append({
            ...
            'prodUrl': f'https://www.amazon.com/dp/{bare_asin}' if bare_asin and bare_asin.startswith('B0') and len(bare_asin) == 10 else None,
        })
    return _write_csv(rows, columns, f'{output_dir}/competitors.csv')


def save_share_of_voice_csv(data, output_dir):
    # SOV returns a single object (not a list), unwrap if wrapped in {"data": ...}
    if isinstance(data, dict) and 'data' in data:
        inner = data['data']
        # SOV data.data is typically a single object, not a list
        data = inner[0] if isinstance(inner, list) and len(inner) == 1 else inner if isinstance(inner, dict) else data
