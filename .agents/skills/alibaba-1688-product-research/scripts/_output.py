...
def format_products_table(products: List[Dict], header: Optional[str] = None) -> str:
    """
    将商品列表格式化为 Markdown 表格（7列版，含缩略图）
    ...
    # 表格前空行（确保 Markdown 渲染器识别表格起始）
    lines.append("")
    
    # 8列表头（含图片、销量）
    lines.append("| 序号 | 图片 | 商品名称 | 价格 | 销量 | 供应商 | 服务与卖点 | 链接 |")
    lines.append("|:-:|:----:|:-----|------:|------:|:------|:------|:----:|")
    
    # 表格内容
    for idx, p in enumerate(valid_products, 1):
        # 图片：使用 image_url，清理查询参数
        image_url = _clean_url(p.get('image_url', ''))
        img_cell = f"![{idx}]({image_url})" if image_url else "-"
        ...
        sold_str = _format_number(sold) if sold else "-"
        ...
        lines.append(f"| {idx} | {img_cell} | {title} | {price_str} | {sold_str} | {supplier} | {tag_cell} | {link_cell} |")
