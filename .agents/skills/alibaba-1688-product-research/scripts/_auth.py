...
from _const import SKILL_VERSION, OPENCLAW_CONFIG_PATH, OPENCLAW_CONFIG_CANDIDATES, AK_SKILL_NAMES
...
def _get_ak_raw_from_config() -> Optional[str]:
    """从所有候选配置路径中读取 AK，按 AK_SKILL_NAMES 优先级遍历。
    遍历 OPENCLAW_CONFIG_CANDIDATES 中所有路径，确保写和读在不同环境时也能命中。
    读取逻辑与 configure/service.py 的 check_existing_config 保持一致。"""
    for config_path in OPENCLAW_CONFIG_CANDIDATES:
        ...
        try:
            ...
            for name in AK_SKILL_NAMES:
                ...
                legacy = skill.get("env", {}).get("ALI_1688_AK", "")
                if legacy:
                    return legacy
        except Exception:
            continue
