...
export function readJsonArgument(args, label = "input") {
  let source
  try {
    source = args.json ?? (args.input === "-" ? readFileSync(0, "utf8") : readFileSync(args.input, "utf8"))
  } catch (error) {
  ...
  const result = spawn("shopify", cliArgs, {
    ...
    env: {
      ...
      SHOPIFY_CLI_AGENT_INFO:
        ...
        `n:shopify-collection-management|v:${SCRIPT_VERSION}|p:none|m:none`,
    },
...
export function throwOnUserErrors(payload, label) {
