...
test('opening uses the animation asset as a gently cropped spherical layer', () => {
  const globe = fs.readFileSync(openingGlobePath, 'utf8');
...
test('transmission copy resolves to DOM nodes before Anime.js reveals it', () => {
  const jsx = fs.readFileSync(openingJsxPath, 'utf8');

  assert.match(jsx, /const dashProjectionCopy = dashProjectionTargets\.flatMap/);
  assert.match(jsx, /const aeroProjectionCopy = aeroProjectionTargets\.flatMap/);
  assert.doesNotMatch(jsx, /const (?:dash|aero)ProjectionCopy = \(\) =>/);
});
...
test('opening stage is gracefully centered and fits viewport with padding', () => {
